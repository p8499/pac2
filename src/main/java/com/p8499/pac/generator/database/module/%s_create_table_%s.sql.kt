...
fun createTable(project: Map<*, *>, folder: File) {
    ...
    project.listItem("modules").map { it as Map<*, *> }.forEachIndexed { index, module ->
        ...
        File(folder, "${module.stringItem("dataSource")}_create_table_${module.stringItem("id")}.sql").apply {
            if (!parentFile.exists()) parentFile.mkdirs()
            writeText(velocityEngine["vm/database/module/%s_create_table_%s.sql.vm"].render(context))
