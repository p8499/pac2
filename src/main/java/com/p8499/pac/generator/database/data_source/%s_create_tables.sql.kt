...
fun createTables(project: Map<*, *>, folder: File) {
    ...
    project.mapItem("envJtee").listItem("dataSources").map { it as Map<*, *> }.forEachIndexed { index, dataSource ->
        ...
        File(folder, "${dataSource.stringItem("id")}_create_tables.sql").apply {
            if (!parentFile.exists()) parentFile.mkdirs()
            writeText(velocityEngine["vm/database/data_source/%s_create_tables.sql.vm"].render(context))
