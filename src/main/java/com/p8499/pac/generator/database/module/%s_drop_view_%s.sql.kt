...
fun dropView(project: Map<*, *>, folder: File) {
    ...
    project.listItem("modules").map { it as Map<*, *> }.forEachIndexed { index, module ->
        ...
        File(folder, "${module.stringItem("dataSource")}_drop_view_${module.stringItem("id")}.sql").apply {
            if (!parentFile.exists()) parentFile.mkdirs()
            writeText(velocityEngine["vm/database/module/%s_drop_view_%s.sql.vm"].render(context))
