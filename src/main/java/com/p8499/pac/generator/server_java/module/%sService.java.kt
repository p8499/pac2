...
fun service(project: Map<*, *>, folder: File) {
    ...
    project.listItem("modules").map { it as Map<*, *> }.forEachIndexed { index, module ->
        ...
        File(folder, "src/main/java/${project.mapItem("envJtee").stringItem("package").replace('.','/')}/service/${module.stringItem("alias")}Service.java").apply {
            if (!parentFile.exists()) parentFile.mkdirs()
            writeText(velocityEngine["vm/server_java/module/%sService.java.vm"].render(context))
            formatJava()
        }
    }
}