...
fun filterDeserializer(project: Map<*, *>, folder: File) {
    ...
    File(folder, "src/main/java/${project.mapItem("envJtee").stringItem("package").replace('.', '/')}/FilterDeserializer.java").apply {
        if (!parentFile.exists()) parentFile.mkdirs()
        writeText(velocityEngine["vm/server_java/base/FilterDeserializer.java.vm"].render(context))
        formatJava()
    }
}