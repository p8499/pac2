...
fun filterLogicExpr(project: Map<*, *>, folder: File) {
    ...
    File(folder, "src/main/java/${project.mapItem("envJtee").stringItem("package").replace('.', '/')}/FilterLogicExpr.java").apply {
        if (!parentFile.exists()) parentFile.mkdirs()
        writeText(velocityEngine["vm/server_java/base/FilterLogicExpr.java.vm"].render(context))
        formatJava()
    }
}