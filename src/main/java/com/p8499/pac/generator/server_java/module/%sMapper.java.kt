...
fun mapper(project: Map<*, *>, folder: File) {
    ...
    project.listItem("modules").map { it as Map<*, *> }.forEachIndexed { index, module ->
        ...
        File(folder, "src/main/java/${project.mapItem("envJtee").stringItem("package").replace('.', '/')}/mapper/${module.stringItem("dataSource")}/${module.stringItem("alias")}Mapper.java").apply {
            if (!parentFile.exists()) parentFile.mkdirs()
            writeText(velocityEngine["vm/server_java/module/%sMapper.java.vm"].render(context))
            formatJava()
        }
    }
}