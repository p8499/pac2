...
fun controllerBase(project: Map<*, *>, folder: File) {
    ...
    project.listItem("modules").map { it as Map<*, *> }.forEachIndexed { index, module ->
        ...
        File(folder, "src/main/java/${project.mapItem("envJtee").stringItem("package").replace('.', '/')}/controller/base/${module.stringItem("alias")}ControllerBase.java").apply {
            if (!parentFile.exists()) parentFile.mkdirs()
            writeText(velocityEngine["vm/server_java/module/%sControllerBase.java.vm"].render(context))
            formatJava()
        }
    }
}