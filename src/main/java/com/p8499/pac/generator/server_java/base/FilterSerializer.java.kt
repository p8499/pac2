...
fun filterSerializer(project: Map<*, *>, folder: File) {
    ...
    File(folder, "src/main/java/${project.mapItem("envJtee").stringItem("package").replace('.', '/')}/FilterSerializer.java").apply {
        if (!parentFile.exists()) parentFile.mkdirs()
        writeText(velocityEngine["vm/server_java/base/FilterSerializer.java.vm"].render(context))
        formatJava()
    }
}