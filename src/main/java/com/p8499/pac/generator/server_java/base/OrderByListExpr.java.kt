...
fun orderByListExpr(project: Map<*, *>, folder: File) {
    ...
    File(folder, "src/main/java/${project.mapItem("envJtee").stringItem("package").replace('.', '/')}/OrderByListExpr.java").apply {
        if (!parentFile.exists()) parentFile.mkdirs()
        writeText(velocityEngine["vm/server_java/base/OrderByListExpr.java.vm"].render(context))
        formatJava()
    }
}