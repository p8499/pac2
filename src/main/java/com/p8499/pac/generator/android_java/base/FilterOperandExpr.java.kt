...
fun filterOperandExpr(project: Map<*, *>, folder: File) {
    ...
    File(folder, "src/main/java/${project.mapItem("envAndroid").stringItem("package").replace('.', '/')}/FilterOperandExpr.java").apply {
        if (!parentFile.exists()) parentFile.mkdirs()
        writeText(velocityEngine["vm/android_java/base/FilterOperandExpr.java.vm"].render(context))
        formatJava()
    }
}