...
fun bean(project: Map<*, *>, folder: File) {
    ...
    project.listItem("modules").map { it as Map<*, *> }.forEachIndexed { index, module ->
        ...
        File(folder, "src/main/java/${project.mapItem("envAndroid").stringItem("package").replace('.', '/')}/bean/${module.stringItem("alias")}.java").apply {
            if (!parentFile.exists()) parentFile.mkdirs()
            writeText(velocityEngine["vm/android_java/module/%s.java.vm"].render(context))
            formatJava()
        }
    }
}