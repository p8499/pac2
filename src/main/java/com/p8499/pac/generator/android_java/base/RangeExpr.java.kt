...
fun rangeExpr(project: Map<*, *>, folder: File) {
    val context = VelocityContext().apply {
        put("project", project)
    }
    File(folder, "src/main/java/${project.mapItem("envAndroid").stringItem("package").replace('.', '/')}/RangeExpr.java").apply {
        if (!parentFile.exists()) parentFile.mkdirs()
        writeText(velocityEngine["vm/android_java/base/RangeExpr.java.vm"].render(context))
        formatJava()
    }
}