...
fun filterExpr(project: Map<*, *>, folder: File) {
    ...
    File(folder, "src/main/java/${project.mapItem("envAndroid").stringItem("package").replace('.', '/')}/FilterExpr.java").apply {
        if (!parentFile.exists()) parentFile.mkdirs()
        writeText(velocityEngine["vm/android_java/base/FilterExpr.java.vm"].render(context))
        formatJava()
    }
}