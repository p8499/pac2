...
fun stub(project: Map<*, *>, folder: File) {
    ...
    project.listItem("modules").map { it as Map<*, *> }.forEachIndexed { index, module ->
        ...
        File(folder, "src/main/java/${project.mapItem("envAndroid").stringItem("package").replace('.', '/')}/stub/${module.stringItem("alias")}Stub.java").apply {
            if (!parentFile.exists()) parentFile.mkdirs()
            writeText(velocityEngine["vm/android_java/module/%sStub.java.vm"].render(context))
            formatJava()
        }
    }
}