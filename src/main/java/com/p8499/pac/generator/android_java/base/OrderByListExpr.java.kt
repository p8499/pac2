...
fun orderByListExpr(project: Map<*, *>, folder: File) {
    val context = VelocityContext().apply {
        put("project", project)
    }
    File(folder, "src/main/java/${project.mapItem("envAndroid").stringItem("package").replace('.', '/')}/OrderByListExpr.java").apply {
        if (!parentFile.exists()) parentFile.mkdirs()
        writeText(velocityEngine["vm/android_java/base/OrderByListExpr.java.vm"].render(context))
        formatJava()
    }
}